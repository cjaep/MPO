...
class ScriptArguments:
    tokenizer_model: Optional[str] = field(default="", metadata={"help": "the tokenizer model"})
...
def prepare_sample_text(example):
    ...
    if "xsum" in script_args.dataset_name:
        # print("SFT for XSUM dataset")
        text = f"Document: {example['document']}\n\nSummary: {example['summary']}"
    elif "tldr" in script_args.dataset_name:
        # print("SFT for TLDR dataset")
        text = f"{example['prompt']}{example['label']}"
    elif "cnn" in script_args.dataset_name:
        # print("SFT for CNN/DM dataset")
        text = f"Article: {example['article']}\n\nHighlights: {example['highlights']}"

    return text

# def prepare_sample_text_PAD(example):
#     """Prepare the text from a sample of the dataset."""
#     text = f"Document: {example['document']}\n\n[PAD]Summary: {example['summary']}</s>"
#     return text
...
tokenizer = AutoTokenizer.from_pretrained(script_args.tokenizer_model, trust_remote_code=True)
